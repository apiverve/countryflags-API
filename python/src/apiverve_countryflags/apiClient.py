...
class CountryflagsAPIClientError(Exception):
...
class ValidationError(CountryflagsAPIClientError):
...
class CountryflagsAPIClient:
    ...
    VALIDATION_RULES = {"country": {"type": "string", "required": True, "minLength": 2, "maxLength": 2}, "format": {"type": "string", "required": True}, "shape": {"type": "string", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CountryflagsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CountryflagsAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise CountryflagsAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CountryflagsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CountryFlagsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CountryflagsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CountryflagsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CountryflagsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CountryflagsAPIClientError(f"Request failed: {str(e)}")
